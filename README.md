...
- 使用网页内置播放器预览视频；B 站预览由本机媒体服务通过 `yt-dlp` 解析 CDN，默认选择不超过 1080p 的最高可用画质。
- 所有来源使用同一套分析流程：
...
- DeepSeek 对话先用 Flash 检查“精简视频记忆＋完整总结时间线＋最近 5 轮对话”能否准确回答。精简记忆包含标题、来源、时长、BV 号或链接、最多 2000 字简介、完整内容概览和最多 700 字声音概览；时间线按时间排序，最多 24 条。只有证据不足时才按开关进入完整回顾或 SerpAPI 联网检索。
...
- **联网搜索**：开启表示允许按需联网。回答充分性检查认为问题依赖外部资料后，Flash 会结合视频标题、简介、总结、近期对话、语言、地区、日期和缺失事实判断搜索需求并提取关键词。用户明确要求联网，或问题明显依赖天气、新闻、最新数据、官方公告等资料时会强制进入这一阶段。字幕不会传给联网判断模型。
- **完整回顾**：默认关闭。开启表示允许按需读取完整总结、完整字幕或较早历史对话；检索和重排完成后，Flash 会再次判断证据是否足够。关闭时不会读取这些冷存档，明确要求看字幕或回顾的问题会说明缺少相应证据。
...
- 每轮问答会合并本轮实际发生的隐藏 Flash 调用和最终 Flash/Pro 回答调用，包括初始充分性检查、按需回顾规划/证据重排、回顾后复核和按需联网规划。
...
- 获取视频：`yt-dlp` 只解析元数据和 CDN 地址，不下载或保存预览视频；优先选择 AVC 1080p 视频轨和 M4A 音频轨。浏览器通过媒体服务的临时 Range 代理按需读取两条轨道，媒体服务向 B 站 CDN 注入解析得到的 Referer 与 User-Agent，网页负责同步播放并由浏览器缓存。
- 生成总结：另行下载最长边不超过 854px、最大 500 MB 的分析素材。

只支持无需登录即可访问、且你有权下载或分析的公开 UGC。当前不使用账号 Cookie；没有 1080p 权限时会自动选择不超过 1080p 的最高可用画质，也不处理会员、私有、付费或地区受限内容。
