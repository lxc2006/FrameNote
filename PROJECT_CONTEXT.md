...
- 左侧已加入一视频一对话的 D1 历史列表，支持新建、切换、重命名和删除；恢复历史时只读取来源描述、总结、模型名与文本消息，不持久化视频、音轨、关键帧、Blob URL 或临时签名地址。
- 顶栏已加入显示设置，支持浅色/深色、中英文字体和四档字号；设置保存在浏览器 localStorage，右侧总结默认字号已调大。
