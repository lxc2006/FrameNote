...
- `POST /api/model/analyze`：调用 `qwen3.5-omni-plus`，接受视频 URL、音轨、关键帧或转写文本，返回标题、概览、要点、章节、结论与证据索引。
...
- 媒体服务通过受控子进程调用固定版本 `yt-dlp`，只处理无需登录的公开 UGC，最高 720p；下载 DASH 视频流与音频流后用原生 FFmpeg 合并。
- 下载前校验来源、公开可用性、直播状态、时长和预计体积；下载中持续限制字节数；完成后用 ffprobe 校验时长、大小和视频轨并计算 SHA-256。
- 成品通过短期 HMAC 签名 URL 由浏览器直取；浏览器校验协议、有效期、媒体类型、元数据和实际字节数，随后复用现有 FFmpeg WASM 提取音轨与关键帧。
- 已实现任务并发/队列上限、20 分钟执行超时、22 分钟网页总等待、取消整个子进程树、失败即时清理、成品 TTL 清理、精确 CORS 与本机无令牌保护。
- 本机已安装 Python 依赖并检测到 yt-dlp、FFmpeg、ffprobe；健康检查、CORS 预检和非法 BVID 接口冒烟测试通过。

### 工程状态

- Python 编译及 14 项媒体服务单元测试通过。
- 前端生产构建、ESLint、`git diff --check` 与 6 项页面/API 回归测试通过。
...
- 真实 B站端到端下载尚未执行，因为当前没有用户明确提供且确认有权处理的公开视频 BV 号；离线验证和本机服务联调已完成。
...
1. 使用用户有权处理的公开 UGC BV 号完成一次真实下载、音轨/关键帧抽取、Qwen 总结和 DeepSeek 追问链路。
