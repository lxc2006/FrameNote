...
export async function prepareBilibiliVideoPreview(
  ...
  try {
    const response = await mediaApiFetch("/v1/bilibili/preview", {
      method: "POST",
      headers: {
...
export async function downloadBilibiliVideo(
  ...
  try {
    let snapshot = await requestJob("/v1/bilibili/jobs", {
      method: "POST",
    ...
    while (
      ...
      snapshot = await requestJob(`/v1/bilibili/jobs/${jobId}`, {
        signal: options.signal,
      });
      reportServerProgress(snapshot, options.onProgress);
    }
...
async function requestJob(path: string, init?: RequestInit) {
  ...
  try {
    response = await mediaApiFetch(path, {
      ...init,
...
async function cleanupJob(jobId: string) {
  try {
    await mediaApiFetch(`/v1/bilibili/jobs/${jobId}`, {
      method: "DELETE",
